...
const CustomerOrdersScreen = () => {
    ...
    if (!orders || orders.length === 0) {
        return <Text style={styles.noOrdersText}>No orders found for this customer.</Text>;
    }
        ...
        <FlatList
            ...
            renderItem={({ item }) => (
                <View style={styles.card}>
                    ...
                    <Text style={styles.cardText}>Product: {item.produce_category.name}</Text>
                    <Text style={styles.cardText}>Quantity: {item.quantity.toFixed(2)}</Text>
